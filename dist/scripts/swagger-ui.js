...
angular.module('swaggerUi').run(['$templateCache', function($templateCache) {
  $templateCache.put('templates/endpoint.html',
    '<div id="{{api.name}}" class="clearfix"> <ul id="{{api.name}}*" class="list-inline pull-left endpoint-heading"> <li> <h4> <a ng-click="api.open=!api.open;permalink(api.name)" ng-bind="api.name"></a> <span ng-if="api.description"> : <span ng-bind="api.description"></span></span> </h4> </li> </ul> <ul class="list-inline pull-right endpoint-actions"> <li> <a ng-click="api.open=!api.open;permalink(api.name)" swagger-translate="endPointToggleOperations"></a> </li> <li> <a ng-click="expand(api);permalink(api.name)" swagger-translate="endPointListOperations"></a> </li> <li> <a ng-click="expand(api,true);permalink(api.name+\'*\')" swagger-translate="endPointExpandOperations"></a> </li> </ul> </div> <ul class="list-unstyled operations" ng-if="api.open"> <li ng-repeat="op in api.operations track by $index" class="operation {{op.httpMethod}}" ng-include="\'templates/operation.html\'"></li> </ul>');
  ...
  $templateCache.put('templates/json-form2/json-form.html',
    '<div> <ul class="list-inline schema"> <li><a ng-click="jsonMode=true" ng-class="{active:jsonMode}">JSON</a></li> <li><a ng-click="jsonMode=false" ng-class="{active:!jsonMode}">form</a></li> </ul> <div ng-if="jsonMode"> <textarea id="param{{param.id}}" ng-model="$parent.modelJson" ng-required="jsonMode&&param.required"></textarea> </div> <div ng-if="!jsonMode"> <div ng-init="property=param.jsonForm" ng-include="\'templates/json-form2/form.html\'"></div> </div> </div>');
